...
This module historically contained both pure Black-Scholes math AND the
DB-backed position analysis. The BS pieces (pricing, Greeks, probability
helpers) have been extracted to `app.services.bs_math` — they're imported
back here unchanged so callers that do `from app.services.risk_analysis
import _estimate_delta` continue to work.

What's actually defined in THIS module:
  - Risk dataclasses (PriceScenario, ExitScenario, OpenPositionAnalysis,
    StrategyGroupAnalysis)
  - Position-level analysis (analyze_open_position)
  - Portfolio aggregation (get_portfolio_risk_summary)
  - Multi-leg grouping (group_positions_for_strategy, analyze_position_group)
  - Exit-scenario builders (generate_exit_scenarios, calculate_*_scenario_*)
"""
...
# Pure Black-Scholes math lives in bs_math. Re-export the symbols that
# external callers (tests, main.py debug routes, charts.py) import via
# this module, so the split is API-preserving.
...
async def get_open_positions_analysis(db: AsyncSession) -> list[OpenPositionAnalysis]:
    ...
    for position in valid_positions:
        ...
        # 1. Manual override beats every live source — operator's belief
        #    about IV (or stress-test scenario) takes precedence.
        if position.volatility_override is not None:
            live_iv = float(position.volatility_override)
            iv_source = "override"
...
def group_positions_for_strategy(
    ...
    manual_group_map: Optional[dict[int, int]] = None,
) -> list[list[OptionPosition]]:
    """Group OptionPositions into multi-leg strategies.

    Resolution order:
      1. Manual groups (PositionGroupMember). If a position's id is in
         `manual_group_map`, it's placed with its co-members and the
         heuristic is bypassed for those positions.
      2. Heuristic: same symbol + same expiration + same calendar open day.
         Catches spreads/condors entered as a single order.

    `manual_group_map` is `position_id -> group_id`. Pass None or {} to
    skip the manual layer (matches old behaviour, useful in tests).
    """
    from collections import defaultdict

    manual_map = manual_group_map or {}
    manual_buckets: dict[int, list[OptionPosition]] = defaultdict(list)
    heuristic_pool: list[OptionPosition] = []

    for p in positions:
        gid = manual_map.get(p.id)
        if gid is not None:
            manual_buckets[gid].append(p)
        else:
            heuristic_pool.append(p)

    heuristic_buckets: dict[tuple, list[OptionPosition]] = defaultdict(list)
    for p in heuristic_pool:
        key = (p.contract.symbol, p.contract.expiration, p.open_date.date())
        heuristic_buckets[key].append(p)

    return list(manual_buckets.values()) + list(heuristic_buckets.values())
...
async def get_portfolio_risk_summary(db: AsyncSession) -> dict:
    ...
    # Resolve manual groupings first — they take precedence over the
    # same-day-same-symbol heuristic.
    from app.models import PositionGroupMember as _PGM
    manual_map: dict[int, int] = {}
    res = await db.execute(select(_PGM.position_id, _PGM.group_id))
    for pos_id, gid in res.all():
        manual_map[pos_id] = gid
    ...
    any_unbounded = False
    for group in group_positions_for_strategy(raw_positions, manual_group_map=manual_map):
